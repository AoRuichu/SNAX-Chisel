...
/** Pillar-2 K-sweep companion: runs Configs 1 (E4M3² UE4M4) and
 *  2 (E4M3×INT8 UE8M0) at K ∈ {512, 2048} so that, combined with the
 *  K=8192 main-sweep data, each config has three K points.  This makes
 *  the K-scaling parameter α in `add_err = c · K^α · 2^(-β·M_acc)`
 *  fittable from data rather than assumed.
 *
 *  Appends to macc_calibration.csv (does NOT clobber the main sweep).
 */
class MaccCalibrationKSweepTest extends AnyFunSuite with ChiselScalatestTester {
  ...
  private case class PCfg(label: String, tA: ElementType, tB: ElementType,
                          sT: ScaleType, K: Int)
  // M_acc range: 7..15 (noise + early saturation; skip 17 since clamp-to-15
  // gives the same actual M_acc anyway).
  private val maccSamples = Seq(7, 9, 11, 13, 14, 15)

  // Configs: Config 1 (E4M3² UE4M4) and Config 2 (E4M3×INT8 UE8M0) at the two
  // K values not covered by the main K=8192 sweep.  Combined with K=8192 main
  // data, each config gets three K points for K-scaling (α) fit.
  private val configs = Seq(
    PCfg("E4M3xE4M3_UE4M4_K512",  MXFormats.E4M3, MXFormats.E4M3, ScaleFormats.UE4M4, 512),
    PCfg("E4M3xE4M3_UE4M4_K2048", MXFormats.E4M3, MXFormats.E4M3, ScaleFormats.UE4M4, 2048),
    PCfg("E4M3xINT8_UE8M0_K512",  MXFormats.E4M3, MXFormats.INT8, ScaleFormats.UE8M0, 512),
    PCfg("E4M3xINT8_UE8M0_K2048", MXFormats.E4M3, MXFormats.INT8, ScaleFormats.UE8M0, 2048),
  )
  private val V = 4
  private val cpb = 8
  private val blockSize = 32

  test("Pillar 2 K-sweep: Wilkinson α validation") {
    // APPEND so it sits alongside the main sweep + supplement data.
    ...
    for (cfg <- configs) {
      ...
      for (M_acc <- maccSamples) {
        ...
        // Pre-rq addition error (Wilkinson fit target)
