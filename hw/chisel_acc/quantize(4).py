...
class ScaleFormat:
    ...
    def __init__(self, ebits: int, mbits: int):
        assert 1 <= ebits + mbits <= 8, "ebits + mbits must be between 1 and 8"
...
# Maximum representable FLOAT VALUE for each element format.
# Used by quantize_mx_v3: ideal_block_scale = abs(block_max) / private_element_max.
# This maps the block maximum exactly to the element format's peak, unlike _EMAX
# which aligns to the nearest power-of-2 boundary (2^EMAX ≤ max_val).
_DTYPE_MAX_VAL = {
    'fp8_e5m2': 57344.0,         # 2^15 * (1 + 3/4)
    'fp8_e4m3': 448.0,           # 2^8  * (1 + 6/8)  (NaN-free variant)
    'fp6_e3m2': 28.0,            # 2^4  * (1 + 3/4)
    'fp6_e2m3': 7.5,             # 2^2  * (1 + 7/8)
    'fp4_e2m1': 6.0,             # 2^(3-1) * (1 + 1/2) = 4 * 1.5
    'mxint8':   127.0 / 64.0,   # implicit scale 2^-6; max magnitude = 127/64
    'int8':     127.0,
}
...
def _compute_block_scale_nv(block: np.ndarray,
                             private_elem_max: float,
                             scale_fmt: ScaleFormat) -> tuple:
    """NVFP4 per-block scale: ideal_scale = abs(block_max) / private_element_max.

    Unlike _compute_block_scale (which uses 2^EMAX, a power-of-2 boundary),
    this formula maps the block maximum exactly to private_element_max, fully
    utilizing the element format's dynamic range.
    Example: E2M1 max_val=6.0 vs 2^EMAX=4.0 — NVFP4 uses 50% more range.

    Args:
        block           : 1D float32 array of block elements
        private_elem_max: max representable value of the element format (e.g. 6.0 for E2M1)
        scale_fmt       : ScaleFormat instance to quantize the ideal scale

    Returns:
        (scale_float, scale_raw_uint8)
    """
    max_abs = float(np.max(np.abs(block)))
    if max_abs == 0.0:
        return 0.0, 0
    return scale_fmt.quantize(max_abs / private_elem_max)
...
def quantize_mx_v3(
    ...
    axis: int = 1,
    packed: bool = False,
    scale_format: str = 'UE4M3',
    compute_raw: bool = True,
) -> tuple:
    """NVFP4-style two-level MX quantization (NumPy).

    Two-level scale scheme
    ----------------------
    1. Per-tensor scale (FP32 scalar):
         per_tensor_scale = scale_fmt.max_val × private_elem_max / max(|tensor|)
       Dividing the entire tensor by per_tensor_scale normalises its peak to
         max(|tensor|) / per_tensor_scale = max(|tensor|)^2 / (scale_fmt.max_val × private_elem_max)
       such that the resulting block scales fall within the representable range
       of the scale format.

    2. Per-block scale (quantized to `scale_format`):
         ideal_block_scale = max(|scaled_block|) / private_elem_max
       This maps the block maximum exactly to private_elem_max, making full
       use of the element format's dynamic range (e.g. E2M1 max = 6.0, whereas
       2^EMAX = 4.0 wastes 33 % of the range).

    Dequantisation
    --------------
    q_arr (returned) is in the *original* value domain:
        q_dequant_original = q_dequant_scaled × per_tensor_scale

    Args:
        arr:          2D float32 input array of shape (Row, Col).
        dtype:        Element format (default: 'fp4_e2m1').
        block_size:   Elements per block along the chosen axis (default: 16).
        axis:         0 → row-wise blocks; 1 → column-wise blocks (default).
        packed:       If True, tightly bit-pack the raw element output.
        scale_format: Block-scale encoding (default: 'UE4M3').
        compute_raw:  Kept for API symmetry with quantize_mx_v2 (always True).

    Returns:
        q_arr            (float32 ndarray, same shape as arr):
                         dequantised values in the *original* domain.
        scale_shared     (float32 ndarray [Row, NB] or [NB, Col]):
                         quantised block scale in the normalised (scaled) domain.
        scale_raw        (uint8 ndarray, same shape as scale_shared):
                         raw 7-bit UE4M3 encodings (MSB always 0, fits uint8).
        q_raw            (uint8/int8 ndarray [Row, Col]):
                         per-element raw encodings (bit-packed when packed=True).
        per_tensor_scale (float32 scalar):
                         FP32 per-tensor normalisation factor.
    """
    arr = np.asarray(arr, dtype=np.float32)
    if arr.ndim != 2:
        raise ValueError(f"quantize_mx_v3 expects a 2D array, got shape {arr.shape}")
    if dtype not in VALID_DTYPES:
        raise ValueError(f"Unknown dtype {dtype!r}. Choose from {VALID_DTYPES}")
    if scale_format not in VALID_SCALE_FORMATS:
        raise ValueError(f"Unknown scale_format {scale_format!r}. Choose from {list(VALID_SCALE_FORMATS)}")
    ...
    # ── 1. Per-tensor scale ────────────────────────────────────────────────
    tensor_abs_max = float(np.max(np.abs(arr)))
    if tensor_abs_max == 0.0:
        per_tensor_scale = np.float32(1.0)
    else:
        per_tensor_scale = np.float32(
            scale_fmt.max_val * private_elem_max / tensor_abs_max
        )

    # ── 2. Normalise entire tensor (divide by per_tensor_scale) ───────────
    arr_scaled = arr / per_tensor_scale

    # ── 3. Block quantisation (on normalised tensor) ───────────────────────
    Row, Col = arr.shape
    raw_dtype = np.int8 if dtype == 'int8' else np.uint8
    q_arr_scaled = np.zeros_like(arr_scaled)
    raw_arr      = np.zeros((Row, Col), dtype=raw_dtype)
    ...
    if axis == 1:
        ...
        for i in range(Row):
            for b in range(n_blocks):
                j0 = b * block_size
                j1 = min(j0 + block_size, Col)
                block = arr_scaled[i, j0:j1]
                sc_f, sc_r = _compute_block_scale_nv(block, private_elem_max, scale_fmt)
                scale_shared[i, b] = sc_f
                scale_raw[i, b]    = sc_r
                q_arr_scaled[i, j0:j1], raw_arr[i, j0:j1] = (
                    _quantize_block_by_scale(block, dtype, sc_f)
                )
        ...
        for b in range(n_blocks):
            ...
            i1 = min(i0 + block_size, Row)
            for j in range(Col):
                block = arr_scaled[i0:i1, j]
                sc_f, sc_r = _compute_block_scale_nv(block, private_elem_max, scale_fmt)
                scale_shared[b, j] = sc_f
                scale_raw[b, j]    = sc_r
                q_arr_scaled[i0:i1, j], raw_arr[i0:i1, j] = (
                    _quantize_block_by_scale(block, dtype, sc_f)
                )
    ...
    # ── 4. Map dequantised values back to original domain ─────────────────
    q_arr = q_arr_scaled * per_tensor_scale

    # ── 5. Optional bit-packing ────────────────────────────────────────────
    if packed and dtype not in ('fp8_e4m3', 'fp8_e5m2', 'int8'):
        packed_rows = [_pack_raw_1d(raw_arr[i], dtype) for i in range(Row)]
        q_raw = np.stack(packed_rows, axis=0)
    else:
    ...
    return q_arr, scale_shared, scale_raw, q_raw, per_tensor_scale
