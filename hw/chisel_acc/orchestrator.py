...
# quantize_mode → output type string (used for Chisel gen directory naming).
# Mode 0 is FP32 pass-through (no requant block); modes 1..6 carry a requant.
_REQUNAT_TYPE_MAP = {0: "fp32", 1: "bf16", 2: "fp8_e5m2", 3: "fp8_e4m3", 4: "mxint8", 5: "fp6_e2m3", 6: "fp6_e3m2"}

# quantize_mode → (Chisel output element tag, output element width in bits) — used by the SV adaptor.
# FP32: 32-bit pass-through (no shared scale, no block); BF16: 16-bit (no shared scale);
# FP8 / mxint8: 8-bit elements with shared scale; FP6: 6-bit elements with shared scale.
_REQUANT_OUT_TAG = {
    0: ("FP32", 32),
    1: ("BF16", 16),
    2: ("E5M2",  8),
