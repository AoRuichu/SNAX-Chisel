...
# quantize_mode → output-type label (just for log lines).
# Mode 0 is FP32 pass-through; modes 1..6 are requant variants.
_REQUANT_LABEL = {
    0: "fp32",
    1: "bf16",
    2: "fp8_e5m2",
...
# Element bit widths — keep in sync with orchestrator.py's _DTYPE_BITS.
_DTYPE_BITS = {
...
# requant_mode → (out-tag for default dir name, output element width in bits)
_REQUANT_OUT_TAG = {
    0: ("FP32",         32),  # FP32 pass-through: result is the FP32 array
    1: ("BF16",         16),  # BF16: 16 bits per element
    2: ("E5M2",          8),  # FP8
    3: ("E4M3",          8),  # FP8
    4: ("INT8",          8),  # mxint8
    5: ("E2M3",          6),  # FP6
...
def _default_out_dir(p: dict) -> Path:
    ...
    tag = out_tag if rqmode in (0, 1) else f"{out_tag}_blk{blk_size}"
    name = (
...
def emit_pe_array_wrapper_sv(p: dict, out_dir: Path) -> Path:
    ...
    # Production-mode PE_Array (cfg.exposeResults=false) has no io_results_o
    # ports. Tests construct PEArrayConfig with exposeResults=true directly in
    # Scala and don't route through this SV adaptor.
