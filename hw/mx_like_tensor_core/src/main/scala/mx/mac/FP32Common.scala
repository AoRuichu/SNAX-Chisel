...
// ============================================================
/** Merges ScaleComposition + ScaleToFP32 into a single module for the UE8M0 scale type.
 *
 *  When mantScaleWidth == 0 (UE8M0), both scale factors are pure powers of 2:
 *    scaleA = 2^(fieldA − bias),  scaleB = 2^(fieldB − bias)
 *  Their mantissa product is trivially 1×1 = 1, so ScaleComposition passes the tree
 *  mantissa through unchanged (zero-padded by 2 bits).  Consequently:
 *
 *    • ScaleComposition mantissa path eliminates both multipliers.
...
// ============================================================
// ScaleComposition output → IEEE FP32 conversion (shared submodule)
// ============================================================
/** Converts the (sign, exp, mant) output of ScaleComposition to a
...
class FPNAdder(val mantBits: Int) extends Module {
  ...
  // Align smaller operand: G_TOTAL guard bits below mantissa.
  // G_TOTAL = M+3 preserves the smaller operand to M_acc-bit precision
  // when shifted by up to M_acc bits (same principle as FusedScaleAcc);
  // matches Cuyckens' 2M+7 shift budget in the mantissa-limit case.
  private val G_RNE   = 3
  private val G_TOTAL = mantBits + G_RNE                   // was hardcoded 3
  val absExpDiff      = Mux(aGreater, expDiff.asUInt, (-expDiff).asUInt)
  val nearM_G         = Cat(nearM, 0.U(G_TOTAL.W))         // (2*mantBits+4) bits
  val alignedNearM    = nearM_G >> absExpDiff
  val stickyFromAlign = (nearM_G & ((1.U << absExpDiff) - 1.U)(mantBits + G_TOTAL, 0)).orR

  val isSub  = valA_S ^ valB_S
  val farM_G = Cat(farM, 0.U(G_TOTAL.W))                   // (2*mantBits+4) bits
  val resMag: UInt = Mux(
    isSub,
    Cat(0.U(1.W), farM_G) - Cat(0.U(1.W), alignedNearM),  // (2*mantBits+5) bits
    farM_G +& alignedNearM                                   // (2*mantBits+5) bits (carry captured)
  )

  val resSign = Mux(aGreater, valA_S, valB_S)

  // Normalize: left-shift until implicit-1 is at bit (mantBits + G_TOTAL + 1)
  val resLZC    = PriorityEncoder(resMag.asBools.reverse)
  val normShift = resMag << resLZC

  // [mantBits+G_TOTAL+1]=implicit-1, [mantBits+G_TOTAL : G_TOTAL+1]=mantissa,
  // [G_TOTAL]=G, [G_TOTAL-1]=R, [G_TOTAL-2:0]=S
  val mantN     = normShift(mantBits + G_TOTAL, G_TOTAL + 1)   // mantBits bits
  val guardBit  = normShift(G_TOTAL)
  val roundBit  = normShift(G_TOTAL - 1)
  val stickyBit = normShift(G_TOTAL - 2, 0).orR || stickyFromAlign
...
// ============================================================
// FusedScaleAccumulator: raw ScaleComposition term → FPn accumulate
// ============================================================
/** Fuses the post-scale-composition normalisation into the FP accumulator
 *  (figure stage (4)/(5): "Alignment(E_SA+E_SW) → + → LZD Norm + RNE").
 *
 *  align + add + LZD + RNE.  The scaled dot-product term enters
 *  as the RAW fixed-point product straight out of ScaleComposition:
 *
 ...
 *  @param mantBits  accumulator mantissa width (== actualAccMantBits).
 *  @param termMantW raw-term mantissa width (== ScaleComposition.effectiveOutMantW).
 *  @param termExpW  raw-term exponent width (== ScaleComposition.effectiveOutExpW).
 */
class FusedScaleAccumulator(
  val scfg:      ScaleAddConfig,
  val mantBits:  Int,
  val termMantW: Int,
  val termExpW:  Int
) extends Module {
  require(mantBits >= 1 && mantBits <= 23, s"FusedScaleAccumulator: mantBits=$mantBits out of [1,23]")
  private val accW = 1 + 8 + mantBits

  override def desiredName =
    s"FusedScaleAcc_${scfg.elementTypeA.name}_x_${scfg.elementTypeB.name}_${scfg.stype.name}_acc${mantBits}b"

  val io = IO(new Bundle {
    val accIn    = Input(UInt(accW.W))       // previous accumulator (FPn: {s, e8, m})
    val termSign = Input(UInt(1.W))
    val termExp  = Input(SInt(termExpW.W))   // ScaleComposition ExpAdd
    val termMant = Input(UInt(termMantW.W))  // raw fixed-point product (un-normalised)
    val accOut   = Output(UInt(accW.W))      // next accumulator (FPn)
  })

  // ── Term exponent bookkeeping (same convention as the deleted ScaleToFPn) ──
  private def elemFrac(t: ElementType): Int =
    if (t.name == "INT8") 0 else t.elementWidthMant
  private val fracBits = elemFrac(scfg.elementTypeA) + elemFrac(scfg.elementTypeB) +
                         2 * scfg.stype.mantScaleWidth
  // Biased exponent of the term's TOP mantissa bit (bit termMantW-1):
  private val topBitBias = 127 + termMantW - 1 - fracBits

  // ── Pre-normalize term (cheap 2-bit LZD + shift) ─────────────────────────
  // ScaleComposition emits (M_A × M_B) × treeMant with M_A, M_B, treeMant all
  // in [1, 2), so the raw product is in [1, 8) and has 0, 1, or 2 leading
  // zeros in the fixed-point representation.  Without pre-normalization,
  //   * etBiased (biased exp of the *top* mantissa bit) overstates the real
  //     leading-1 exp by up to 2, causing:
  //     - Over-alignment of the OTHER operand by up to 2 bits (precision loss);
  //     - Top 2 field positions storing zeros (wasted field precision);
  //     - The aGreater fallback (accG >= termG) biased against an unnormalized
  //       term whose real magnitude may exceed accreg's.
  // Fix: detect leading zeros from the top two bits and shift-left in one cycle
  // (2-bit LZD), then subtract the shift amount from etBiased.
  private val termTopBit    = io.termMant(termMantW - 1)
  private val termSecondBit = io.termMant(termMantW - 2)
  val termLeadZeros = Mux(termTopBit,   0.U(2.W),
                       Mux(termSecondBit, 1.U(2.W),
                                          2.U(2.W)))
  val termNormMant = (io.termMant << termLeadZeros)(termMantW - 1, 0)
  val etBiased     = io.termExp +& topBitBias.S - termLeadZeros.zext

  // ── Accumulator unpack ──
  val accSign  = io.accIn(accW - 1)
  val accEUI   = io.accIn(accW - 2, mantBits)          // 8-bit biased exp field
  val accMant  = Cat(accEUI.orR, io.accIn(mantBits - 1, 0))  // mantBits+1, implicit-1 at MSB
  val eaBiased = accEUI.zext                            // SInt — acc top-bit exp

  val termZero = io.termMant === 0.U
  val accZero  = accMant === 0.U

  // ── Left-justify both mantissas to a common field (MSB = top bit), + G guards ──
  // G derivation: to preserve the smaller operand to M_acc-bit precision after
  // an alignment shift of absDiff, need
  //     fieldMantW - 1 - absDiff - M_acc  ≥  0
  // i.e. the shift budget must scale with M_acc, not stay a small constant.
  // G_RNE = 3 covers GRS for the post-align RNE rounding; matches Cuyckens'
  // 2M+7 budget in the M-limit.
  private val G_RNE      = 3
  private val fieldMantW = math.max(termMantW, mantBits + 1)
  private val G          = mantBits + G_RNE             // was hardcoded 3
  private val fieldW     = fieldMantW + G               // pre-add width
  val termJust = (termNormMant << (fieldMantW - termMantW))(fieldMantW - 1, 0)
  val accJust  = (accMant     << (fieldMantW - (mantBits + 1)))(fieldMantW - 1, 0)
  val termG    = Cat(termJust, 0.U(G.W))               // fieldW bits, top bit @ exp etBiased
  val accG     = Cat(accJust,  0.U(G.W))               // fieldW bits, top bit @ exp eaBiased

  // ── Pick larger by top-bit biased exponent (zero-guarded so a zero term
  //    can never become the reference and wipe the accumulator) ──
  val aGreater = termZero ||
                 (!accZero && (eaBiased > etBiased ||
                              (eaBiased === etBiased && accG >= termG)))

  val farExp   = Mux(aGreater, eaBiased, etBiased)     // biased exp of larger top bit
  val farM_G   = Mux(aGreater, accG,  termG)
  val nearM_G  = Mux(aGreater, termG, accG)
  val farSign  = Mux(aGreater, accSign,     io.termSign)
  val nearSign = Mux(aGreater, io.termSign, accSign)

  // ── Align smaller operand (single barrel shift), sticky-collect ──
  val expDiffS = eaBiased - etBiased
  val absDiff  = Mux(expDiffS < 0.S, (-expDiffS).asUInt, expDiffS.asUInt)
  val shAmt    = Mux(absDiff > fieldW.U, fieldW.U, absDiff)  // clamp: beyond field → pure sticky
  val alignedNear = nearM_G >> shAmt
  val stickyAlign = (nearM_G & ((1.U << shAmt) - 1.U)(fieldW - 1, 0)).orR

  // ── Add / subtract ──
  // The term is UNNORMALISED (≤2 leading zeros from (M_SA×M_SB)×treeMant), so
  // the operand chosen "far" by top-bit exponent is NOT guaranteed to have the
  // larger magnitude.  A plain farM_G − alignedNear can therefore borrow; detect
  // it (MSB of the widened result), take the 2's-complement magnitude, and adopt
  // the near operand's sign — same scheme as CustomFPAdder.
  val isSub     = (farSign ^ nearSign).asBool
  val addMag    = farM_G +& alignedNear                             // fieldW+1 bits
  val subFwd    = Cat(0.U(1.W), farM_G) - Cat(0.U(1.W), alignedNear) // fieldW+1 bits
  val subBorrow = subFwd(fieldW).asBool                            // set ⇒ near > far
  val subMag    = Mux(subBorrow, (~subFwd + 1.U)(fieldW, 0), subFwd)
  val resMag    = Mux(isSub, subMag, addMag)                       // fieldW+1 bits
  private val W = fieldW + 1
  val resSign   = Mux(isSub && subBorrow, nearSign, farSign)

  // ── Single LZD normalise ── (leading-1 driven up to bit W-1)
  val resLZC    = PriorityEncoder(resMag.asBools.reverse)
  val normShift = (resMag << resLZC)(W - 1, 0)

  // implicit-1 @ bit W-1; mantissa [W-2 : W-1-mantBits]; then G/R/S
  val mantN     = normShift(W - 2, W - 1 - mantBits)
  val guardBit  = normShift(W - 2 - mantBits)
  val roundBit  = normShift(W - 3 - mantBits)
  val stickyBit = normShift(W - 4 - mantBits, 0).orR || stickyAlign
  ...
  val finalE    = farExp + 1.S - resLZC.zext + mantCarry.zext
  val isZero    = resMag === 0.U || finalE <= 0.S
  val isOverflow = finalE >= 255.S

  io.accOut := Mux(isZero,     0.U(accW.W),
               Mux(isOverflow, Cat(resSign, 255.U(8.W), 0.U(mantBits.W)),
                               Cat(resSign, finalE.asUInt(7, 0), finalM)))
}
...
// ScaleToFPn was removed: the non-UE8M0 path now fuses its LZC+RNE into
// FusedScaleAccumulator (see above).  The FP32 reference variant ScaleToFP32
// is retained for the (non-instantiated) DSE/reference emit.
